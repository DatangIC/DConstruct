...
abstract class NetworkBoundWithoutDbResource<ResponseType, ResultType>
@MainThread constructor(private val appExecutors: AppExecutors) {
    ...
    private fun fetchFromNetwork() {
        ...
        result.addSource(apiResponse) { response ->
            ...
            when (response) {
                    ...
                    appExecutors.diskIO().execute {
                        ...
                        appExecutors.mainThread().execute {
                            ...
                            if (hasNextCall(index)) {
                                fetchFromNetwork()
                                setValue(Resource.loading(data.data))
                            } else {
                                setValue(data)
                            }
                        }
                ...
                is ApiEmptyResponse -> {
                    setValue(Resource.empty("Empty", null))
                }
                is ApiErrorResponse -> {
     ...
     */
    @WorkerThread
    protected abstract fun processResponse(response: ApiSuccessResponse<ResponseType>): Resource<ResultType>
