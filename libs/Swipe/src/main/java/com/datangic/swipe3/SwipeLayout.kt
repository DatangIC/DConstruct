...
class SwipeLayout : ViewGroup {
    private val TAG = "swipe3/SwipeLayout"

    private val mScaleTouchSlop by lazy {
        ViewConfiguration.get(context).scaledTouchSlop //为了处理单击事件的冲突
    }
    private val mMaxVelocity by lazy { ViewConfiguration.get(context).scaledMaximumFlingVelocity } //计算滑动速度用
    ...
    private var isOutSideCancel = true
    ...
    //存储的是当前正在展开的View
    private var mViewCache: SwipeLayout? = null
    ...
    private var isLeftSwipe = true //左滑右滑的开关,默认左滑打开菜单
    ...
    constructor(context: Context, attributeSet: AttributeSet? = null, defStyle: Int = 0) : super(context, attributeSet, defStyle) {
        val ta = context.theme.obtainStyledAttributes(attributeSet, R.styleable.SwipeLayout, defStyle, 0)
        val count = ta.indexCount
        for (i in 0 until count) {
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            when (val attr = ta.getIndex(i)) {
                R.styleable.SwipeLayout_swipeEnable -> {
                    isSwipeEnable = ta.getBoolean(attr, true)
                }
                R.styleable.SwipeLayout_ios -> {
                    isIos = ta.getBoolean(attr, true)
                }
                R.styleable.SwipeLayout_leftSwipe -> {
                    isLeftSwipe = ta.getBoolean(attr, true)
                }
                R.styleable.SwipeLayout_outsideCancel -> {
                    isOutSideCancel = ta.getBoolean(attr, true)
                }
            }
        }
        ta.recycle()
     ...
     * @param ios
     */
    fun setIos(ios: Boolean): SwipeLayout {
        isIos = ios
        return this
    }
     ...
     * @return
     */
    fun setLeftSwipe(leftSwipe: Boolean): SwipeLayout {
...
//        Log.e(TAG, "dispatchTouchEvent ev.scrollX=$scrollX\n")
        if (isSwipeEnable) {
            when (ev.action) {
                MotionEvent.ACTION_DOWN -> {
//                    Log.e(TAG, "onDown ev.scrollX=$scrollX\n")
...
//                    Log.e(TAG, "is Move ev.scrollX=$scrollX\n")
...
/*                    if (!mScroller.isFinished()) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    if (!iosInterceptFlag) { //且滑动了 才判断是否要收起、展开menu
                        //求伪瞬时速度
//                        Log.e(TAG, "is UP ")
                        mVelocityTracker ?: acquireVelocityTracker(ev)
